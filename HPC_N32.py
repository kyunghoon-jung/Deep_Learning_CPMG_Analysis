...
for file_num in range(1):
    ...
    for model_idx, [A_first, A_end, B_first, B_end] in enumerate(model_lists):
        ...
        for idx1, [A_idx, B_idx] in enumerate(AB_idx_set):
            model_index = get_model_index(total_indices, A_idx, time_thres_idx=time_range-20, image_width=image_width)
            model_index = model_index[:cut_idx, :]
            exp_data_32_test = exp_data_32[model_index.flatten()]

            if PRE_PROCESS==True:
                exp_data_32_test = pre_processing(exp_data_32_test, PRE_SCALE)
            else:
                exp_data_32_test = 1-(2*exp_data_32_test - 1)
            exp_data_32_test = exp_data_32_test.reshape(1, -1)
            exp_data_32_test = torch.Tensor(exp_data_32_test).cuda()

            pred = model(exp_data_32_test)
            ...
            exp_data_32_test = exp_data_32_deno_deco[model_index.flatten()]
