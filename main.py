...
if pars_args.evaluateall:
    ...
    hpc_model = HPC_Model(*args)
    total_A_lists, total_raw_pred_list, total_deno_pred_list = hpc_model.binary_classification_train()
    np.save('./data/models/pred_raw_test_{}_{}_3.npy'.format(N_PULSE, B_init), total_raw_pred_list)
    np.save('./data/models/pred_deno_test_{}_{}_3.npy'.format(N_PULSE, B_init),total_deno_pred_list)
    np.save('./data/models/A_lists_test_{}_{}_3.npy'.format(N_PULSE, B_init), total_A_lists)
    # print('Total computational time:', time.time() - tic)
