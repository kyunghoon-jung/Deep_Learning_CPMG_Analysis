...
class Denoise_Model(nn.Module):
    def __init__(self):
        ...
        self.conv1d_1 = nn.Conv1d(1, 64, 4, stride=2, padding=1)
        self.bn1 = nn.BatchNorm1d(64)
        self.conv1d_2 = nn.Conv1d(64, 64, 4, stride=2, padding=1)
        self.bn2 = nn.BatchNorm1d(64)
        self.leakyrelu = nn.LeakyReLU()
        self.conv1d_3 = nn.ConvTranspose1d(64, 64, 4, stride=2, padding=1)
        self.bn3 = nn.BatchNorm1d(64)
        self.conv1d_4 = nn.ConvTranspose1d(64, 1, 4, stride=2, padding=1)
        self.bn4 = nn.BatchNorm1d(1)
    
    def forward(self, x):
        x = self.leakyrelu(self.bn1(self.conv1d_1(x)))
        x = self.leakyrelu(self.bn2(self.conv1d_2(x)))
        x = self.leakyrelu(self.bn3(self.conv1d_3(x)))
        x = self.conv1d_4(x)
        return x       
